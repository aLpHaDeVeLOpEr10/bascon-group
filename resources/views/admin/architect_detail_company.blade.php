...
<script>
    ...
    var table = $('#b_categoer_table').DataTable({
        "ajax": {
            "url": "{{ url('admin_setting/show_company_architect/' . $const_id) }}",
            "type": "GET",
            "dataType": "json",
            "dataSrc": "data"
        },
...
$(document).ready(function() {
        $('#updateForm').submit(function(event) {
            ...
            $.ajax({
                url: "{{ url('admin_setting/update_payment_company') }}",
                type: 'POST',
                data: formData,
                dataType: 'json',
                success: function(response) {
    ...
    function openUpdateModal(userId) {
        ...
        Swal.fire({
            ...
            onBeforeOpen: () => {
                ...
                $.ajax({
                    url: "{{ url('Admin_setting/get_payment_company') }}",
                    type: 'GET',
                    data: {
                        userId: userId
                    },
    ...
    function deleteUser(userId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Send AJAX request
                $.ajax({
                    url: "{{ url('admin_setting/delete_payment_company') }}",
                    type: "POST",
                    data: {
                        userId: userId
                    },
